...
parser = argparse.ArgumentParser(
    prog="oculus_to_bag", description='Extract the .oculus native format and save them in an Oculus sonar ping messages in a ROS bag.')
parser.add_argument('input', type=str,
                    help='Path where to save the .oculus file.')
parser.add_argument('-o', dest='output', type=str, help='Path to rosbag.')
parser.add_argument('-t', dest='topic', type=str, default='',
                    help='Name of the output topic name containing the ping.')
...
parser.add_argument('-U', '--use-input-dir', dest='use_input_dir', default=False, action='store_true',
                    help='Use input dir and auto-generated filename as output path')  # TODO
args = parser.parse_args()

# Checking input path # TODO
if not os.path.isfile(args.input):
    raise ValueError('Invalid input, '+args.input+' : is not a regular file.')
# if using input dir, generating a path # TODO
if args.use_input_dir:
    args.output = os.path.split(args.input)[0]
# Checking output path # TODO
if args.output:
...
with open(args.input, 'rb') as inputOculusFile:
    log_header_struct = (
    ...
    for ii in range(len(log_header_val)):
        inputOculusFile.write(struct.pack(log_header_fmt[ii], log_header_val[ii]))
    ...
    for i, bagMessage in enumerate(bag.read_messages(topics=[args.topic])):
        header_struct = (
            ...
            {'name': 'time_unix', 'type': 'd',
                'val': bagMessage.timestamp.to_time()},
        ...
        for ii in range(len(header_val)):
            inputOculusFile.write(struct.pack(header_fmt[ii], header_val[ii]))

        inputOculusFile.write(bagMessage.message.data)
...
with open(args.output, 'rb') as inputFile:
    log_header_struct = (
        {'name': 'fileHeader', 'type': 'I', 'val': 2037},
        {'name': 'sizeHeader', 'type': 'I', 'val': 48},
        {'name': 'source', 'type': '16s', 'val': "Oculus".encode('ascii')},
    ...
    for ii in range(len(log_header_val)):
        struct.pack(log_header_fmt[ii], log_header_val[ii]) = inputFile.read()
